...
try {
    ...
    $sportResult = $mysql->query('SELECT * FROM `blog_posts` WHERE `type` = "sport" ORDER BY `date_added` DESC');
...
if (!isset($_SERVER['ERROR'])) {
    ?>
    <div
        class="container-fluid container-md <?php if (!(isset($_SERVER['sport_row']) && count($_SERVER['sport_row']) > 0)) { ?>my-auto<?php } ?>">
        <?php
